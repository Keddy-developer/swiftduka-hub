                  ...
                  <ResponsiveContainer width="100%" height="100%">
                     <AreaChart data={stats?.chartData || []}>
                        ...
                        <Tooltip
                           ...
                           itemStyle={{ fontWeight: 'black', fontSize: '10px', textTransform: '', padding: '4px 0' }}
                        />
                  ...
                  {stats?.alerts?.map((alert, idx) => (
                     <div key={alert.id || idx} className="p-6 hover:bg-slate-50 transition-all cursor-pointer border-b border-slate-100 last:border-0 relative">
                        <div className="flex gap-4">
                           <div className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 shadow-lg ${alert.type === 'warning' ? 'bg-amber-500 shadow-amber-200' :
                              alert.type === 'error' ? 'bg-rose-500 shadow-rose-200' : 'bg-blue-500 shadow-blue-200'
                              }`} />
                           <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-start mb-1 gap-2">
                                 <p className="font-black text-[11px] text-slate-900  tracking-tight leading-none truncate">{alert.title}</p>
                                 <span className="text-[9px] font-black text-slate-400  tracking-tighter whitespace-nowrap">{alert.time}</span>
                              </div>
                              <p className="text-[11px] font-bold text-slate-500 leading-relaxed  tracking-tight line-clamp-2 italic">{alert.message}</p>
                           </div>
                        </div>
                     </div>
                  ))}
