      ...
      <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
         ...
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-200 pb-8">
            <div>
               ...
               <p className="text-[10px] md:text-sm font-bold text-slate-400 tracking-[0.2em] mt-1">
                  Manage decentralized logistics infrastructure for {hub?.name}
               </p>
            </div>
            <div className="flex items-center gap-3 bg-blue-50 px-4 py-2 rounded-xl border border-blue-100">
               <Shield className="text-blue-600" size={16} />
               <span className="text-[10px] font-black text-blue-900 tracking-tight">System Managed Infrastructure</span>
            </div>
         </div>
            ...
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
               <div className="bg-white w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden border border-slate-200 animate-in zoom-in-95 duration-300">
                  ...
                  <form onSubmit={handleSubmit} className="p-8 space-y-6">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-slate-500 tracking-widest px-1">Town</label>
                           <input
                              disabled
                              value={formData.town || ''}
                        ...
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-slate-500 tracking-widest px-1">County</label>
                           <input
                              disabled
                     ...
                     {activeTab === 'areas' && (
                        <div className="flex items-center gap-3 p-4 bg-blue-50 border border-blue-100 rounded-2xl">
                           ...
                           />
                           <label htmlFor="free-del" className="text-xs font-black text-blue-900 tracking-tight">Enable Standard Free Delivery Eligibility</label>
                        </div>
                     )}
...
const TabButton = ({ active, onClick, icon: Icon, label }) => (
   <button
      onClick={onClick}
      className={`flex-1 flex items-center justify-center gap-3 py-3 px-4 rounded-xl text-xs font-black tracking-widest transition-all ${active
      ...
      <div className="space-y-4">
         <div>
            <div className="flex items-center gap-2 mb-1">
               <h3 className="text-lg font-black text-slate-900 tracking-tight leading-none">{item.name}</h3>
               <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-sm shadow-emerald-200" />
            </div>
            <p className="text-[10px] font-bold text-slate-400 tracking-widest flex items-center gap-1.5 pt-1">
         ...
         <div className="p-4 bg-slate-50/80 border border-slate-100 rounded-2xl">
            {type === 'stations' ? (
               <div className="space-y-2">
                  <p className="text-[10px] font-black text-slate-900 flex justify-between">
...
const EmptyState = ({ type, hasSearch, onAdd }) => (
   <div className="py-20 flex flex-col items-center text-center max-w-md mx-auto">
      ...
      <p className="text-sm font-bold text-slate-400 mt-4 leading-relaxed tracking-tight">
         {hasSearch
