...
const StatusBadge = ({ status }) => {
   ...
   return (
      <span className={`px-2 py-0.5 rounded text-[10px] font-bold tracking-tight border ${colors[status] || colors.BASIC}`}>
         {status}
      </span>
   );
};
         ...
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            <div className="space-y-1">
               ...
               <h2 className="text-xl md:text-2xl font-bold text-slate-900 tracking-tighter flex items-center gap-3 lowercase">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold tracking-tight border ${seller.approvalStatus === 'approved' ? 'bg-green-50 text-green-700 border-green-100' : 'bg-amber-50 text-amber-700 border-amber-100'}`}>
