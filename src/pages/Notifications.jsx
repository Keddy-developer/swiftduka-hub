      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-3">
            NOTIFICATIONS
            <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-[10px] rounded uppercase">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className="px-4 py-2 bg-white border border-slate-200 rounded text-[11px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center gap-2 disabled:opacity-50 shadow-sm"
          >
         ...
         <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
            {['all', 'unread'].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-tight transition-all whitespace-nowrap ${
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden divide-y divide-slate-100">
        {loading ? (
          ...
          filtered.map((n) => (
            ...
            >
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
                n.type === 'URGENT' ? 'bg-rose-50 text-rose-500' : 
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className={`text-sm font-black uppercase tracking-tight truncate ${!n.isRead ? 'text-slate-900' : 'text-slate-600'}`}>
                    {n.title}
                  </h3>
                ...
                <div className="flex items-center gap-3 mt-2 sm:mt-1">
                   ...
                   {n.type === 'URGENT' && (
                     <span className="text-[10px] font-black text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full uppercase italic animate-pulse">Critical Priority</span>
                   )}
                </div>
              </div>
          ...
          <div className="py-24 text-center">
            ...
            <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight">Zero tactical alerts</h3>
