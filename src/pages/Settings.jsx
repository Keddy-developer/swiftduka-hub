...
const SettingsPage = () => {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center p-20 opacity-50">
      ...
      <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Synchronizing Logistics Parameters...</span>
    </div>
  );
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-slate-200 pb-6 gap-4">
        ...
        <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest border border-slate-200 px-3 py-1.5 rounded-full bg-white">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <aside className="w-full lg:w-64 shrink-0">
          <nav className="flex flex-row lg:flex-col gap-1 overflow-x-auto lg:overflow-visible no-scrollbar p-1 bg-slate-100/50 rounded-lg lg:bg-transparent">
            {tabs.map((tab) => (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded text-xs font-bold uppercase tracking-widest transition-all whitespace-nowrap lg:w-full ${
                  activeTab === tab.id 
          ...
          <div className="mt-8 hidden lg:block p-5 bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl text-white shadow-xl">
             <h4 className="text-[10px] font-black uppercase tracking-[0.2em] mb-3 text-slate-400">System Health</h4>
             <div className="space-y-4">
                <div className="flex items-center justify-between">
                   <span className="text-[10px] font-bold text-slate-400">Connectivity</span>
                   <span className="text-[10px] font-bold text-green-400 uppercase">99.9%</span>
                </div>
                <div className="flex items-center justify-between">
                   <span className="text-[10px] font-bold text-slate-400">Sync Delay</span>
                   <span className="text-[10px] font-bold text-blue-400 uppercase">1.2s</span>
                </div>
                <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                   <div className="h-full w-[85%] bg-blue-500"></div>
                </div>
                <p className="text-[9px] font-medium text-slate-500 italic uppercase">Logistics engine nominal.</p>
             </div>
          </div>
        </aside>
               ...
               <form onSubmit={handleUpdateHub} className="p-6 md:p-8 space-y-8">
                  ...
                  <div className="pt-8 border-t border-slate-100 flex justify-end">
                     <button type="submit" disabled={saving}
                       className="px-8 py-3 bg-slate-900 text-white rounded text-[11px] font-black uppercase tracking-[0.2em] hover:bg-slate-800 transition-all shadow-xl flex items-center gap-3 disabled:opacity-50"
                     >
               ...
               <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6">
                    ...
                    <div key={route.id} className="bg-white border border-slate-200 rounded p-5 group hover:border-blue-500/50 hover:shadow-xl hover:shadow-slate-200/50 transition-all relative overflow-hidden">
                       <div className="flex justify-between items-start mb-4">
                          <div className="flex items-center gap-3">
                             ...
                             <div>
                                <h4 className="text-xs font-black text-slate-900 uppercase pr-8 truncate tracking-wide">{route.toTown}</h4>
                                <span className={`text-[9px] font-bold uppercase tracking-tight px-2 py-0.5 rounded border mt-1 inline-block ${
                                  route.deliveryCategory === 'express' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-blue-50 text-blue-600 border-blue-100'
                                }`}>
                  ...
                  {routes.length === 0 && (
                     <div className="col-span-full py-20 text-center flex flex-col items-center justify-center opacity-30">
                        ...
                        <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Isolated Node: No active routes</p>
                     </div>
                  )}
               </div>
            </div>
            ...
            <div className="p-8 md:p-12 animate-in slide-in-from-right-4 duration-300">
               <div className="max-w-md mx-auto text-center space-y-8">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Authorized Node Manager</p>
                  </div>
                  ...
                  <div className="pt-8 space-y-3">
                     <button className="w-full py-3 bg-white border border-slate-200 rounded text-[11px] font-black uppercase tracking-[0.2em] hover:bg-slate-50 transition-all flex items-center justify-center gap-3">
          ...
          {activeTab === 'ops' && (
            <div className="p-8 animate-in slide-in-from-right-4 duration-300">
               ...
               <div className="mt-12 bg-amber-50 border border-amber-100 p-6 rounded-lg flex gap-4">
                  ...
                  <div>
                    <h4 className="text-xs font-black text-amber-800 uppercase tracking-tight">System Compliance Warning</h4>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
              <div className="px-8 py-6 bg-slate-900 flex items-center justify-between">
                 <div>
                    <h2 className="text-lg font-black text-white uppercase tracking-tight">Deploy Logistics Zone</h2>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Authorized Route Authorization</p>
                 </div>
              ...
              <form onSubmit={handleUpsertRoute} className="p-8 space-y-6 max-h-[80vh] overflow-y-auto no-scrollbar">
                 <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col gap-2">
                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Service Level</label>
                 ...
                 <div className="flex gap-3 pt-6">
                    <button type="button" onClick={() => setShowRouteModal(false)} className="flex-1 py-3 bg-white border border-slate-200 rounded text-[11px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all">Cancel</button>
                    <button type="submit" disabled={saving} className="flex-1 py-3 bg-slate-900 text-white rounded text-[11px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl">
...
const DataUnit = ({ label, value }) => (
  <div>
    ...
    <p className="text-[11px] font-black text-slate-900 uppercase tracking-wide">{value}</p>
  </div>
);
...
const OpsPolicyCard = ({ title, desc, value, active }) => (
  <div className="p-5 border border-slate-200 rounded-lg hover:border-blue-500/50 transition-all bg-white shadow-sm flex flex-col justify-between">
    ...
    <div className="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between">
      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Policy Threshold</span>
      <span className="text-[11px] font-black text-blue-600 uppercase tracking-wide">{value}</span>
    </div>
  </div>
);
